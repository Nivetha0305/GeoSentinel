...
function App() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/map-data?limit=400`)
      .then((res) => {
        if (!res.ok) {
          throw new Error(
            'Map data failed. Check whether the Render backend is running.'
          )
        }
        return res.json()
      })
  ...
  const mapCounts = useMemo(() => {
    ...
    for (const point of points) {
      counts[point.risk_level] =
        (counts[point.risk_level] || 0) + 1
    }
  ...
  async function runPredict(payload = form) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          body.detail ||
            'Prediction failed. Check the Render backend.'
        )
      }
  ...
  return (
    <div className="app">
      <header className="topbar">
        <div>
          <p className="kicker">
            Nilgiris · AI early warning
          </p>

          <h1>GeoSentinel</h1>

          <p className="subtitle">
            Flash flood risk monitoring from rainfall, terrain
            and satellite moisture
            {stats
              ? ` · ${stats.rows.toLocaleString()} records · ${stats.years}`
              : ''}
          </p>
        </div>
        ...
        <div className={`risk-badge ${risk.toLowerCase()}`}>
          ...
          <strong>
            {result ? risk.toUpperCase() : 'SCANNING'}
          </strong>
        ...
        {Object.keys(RISK_COLORS).map((level) => (
          <div
            ...
            className={`level-chip ${
              result?.risk_level === level ? 'active' : ''
            }`}
          >
            <i
              style={{
      ...
      <section className="workspace">
        ...
        >
          ...
          <p className="hint">
            Nilgiris CSV features · map click fills the form
          </p>
          ...
          <Field
            label="Rainfall (mm)"
            value={form.Rainfall}
            onChange={(v) => updateField('Rainfall', v)}
            min="0"
            max="1000"
            step="1"
          />
        ...
        <div className="panel map-panel">
          ...
          {mapLoading ? (
            <p className="hint">
              Loading Nilgiris sample points…
            </p>
          ) : points.length === 0 ? (
            <p className="hint">
              No map points received from backend.
            </p>
          ) : (
            <MapContainer
              center={NILGIRIS_CENTER}
              zoom={9}
              className="map"
            >
              <TileLayer
